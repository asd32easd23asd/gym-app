...
test('new customers start free with onboarding pending and no fabricated personal data',()=>{const d=C.initial();assert.equal(d.settings.onboardingVersion,0);assert.equal(d.settings.premiumPreview,false);assert.equal(d.settings.starterDismissed,false);for(const field of ['products','usages','measurements','photos','reminders'])assert.deepEqual(d[field],[]);assert.deepEqual(d.sessions,{});assert.ok(d.plan.days.every(day=>day.exercises.length===0));});
test('settings upgrade preserves existing data and explicit choices while validating new flags',()=>{const d=C.initial();d.measurements.push({id:'m',date:'2026-09-25',weight:80,note:'Own data'});d.settings={notifications:true};C.upgrade(d);assert.equal(d.settings.premiumPreview,false);assert.equal(d.settings.notifications,true);assert.equal(d.measurements[0].note,'Own data');d.settings.onboardingVersion=1;d.settings.premiumPreview=true;C.upgrade(d);assert.equal(d.settings.onboardingVersion,1);assert.equal(d.settings.premiumPreview,true);for(const [key,value] of [['premiumPreview','true'],['onboardingVersion',2],['starterDismissed',1]]){const bad=JSON.parse(JSON.stringify(d));bad.settings[key]=value;assert.throws(()=>C.validate(bad));}});
